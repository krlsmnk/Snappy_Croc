...
namespace RecordAndPlay.Editor
{
    ...
    public static class RecordingUtil
    {
        ...
        public static AnimationClip[] GetAnimationClips(Recording recording, bool smooth)
        {
            ...
            for (int subjectIndex = 0; subjectIndex < recording.SubjectRecordings.Length; subjectIndex++)
            {
                ...
                for (int i = 0; i < subj.CapturedCustomEvents.Length; i++)
                {
                    // If it was a non-dictionary event
                    if (subj.CapturedCustomEvents[i].Contents.Count == 1 && subj.CapturedCustomEvents[i].Contents.ContainsKey("value"))
                    {
                        animationEvents.Add(new AnimationEvent
                        {
                            time = subj.CapturedCustomEvents[i].Time - subj.GetStartTime(),
                            functionName = subj.CapturedCustomEvents[i].Name,
                            stringParameter = subj.CapturedCustomEvents[i].Contents["value"],
                        });
                    }
                    else
                    {
                        // Create an event per value in dictionary
                        foreach (var item in subj.CapturedCustomEvents[i].Contents)
                        {
                            animationEvents.Add(new AnimationEvent
                            {
                                time = subj.CapturedCustomEvents[i].Time - subj.GetStartTime(),
                                functionName = string.Format("{0}-{1}", subj.CapturedCustomEvents[i].Name, item.Key),
                                stringParameter = item.Value,
                            });
                        }
                    }
                }
                AnimationUtility.SetAnimationEvents(clips[subjectIndex], animationEvents.ToArray());
                ...
                clips[subjectIndex].EnsureQuaternionContinuity();
            }
