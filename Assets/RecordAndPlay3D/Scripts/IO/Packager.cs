...
namespace RecordAndPlay.IO
{
    ...
    public static class Packager
    {
        ...
        /// </summary>
        /// <param name="path">Path to store recordings.</param>
        /// <param name="recordings">The recordings to save to disk.</param>
        public static long SaveRecordingsToFile(string path, params Recording[] recordings)
        {
            using (FileStream fs = File.Create(path))
            {
                return Package(fs, recordings);
            }
        }
        ...
        /// <returns>When overridden in a derived class, gets the length in bytes of the stream.</returns>
        public static long Package(Stream outStream, params Recording[] recordings)
        {
            ...
            using (BinaryWriter writer = new BinaryWriter(outStream, new System.Text.UTF8Encoding(false, true), true))
            {
                writer.Write((byte)1);
            ...
            for (int recordingIndex = 0; recordingIndex < recordings.Length; recordingIndex++)
            {
                ...
                for (int vIndex = 0; vIndex < recordings[recordingIndex].CapturedLifeCycleEvents.Length; vIndex++)
                {
                    transports[recordingIndex].LifecycleEvents.Add(new Transport.LifeCycleEventCapture()
                    {
                        ...
                        Type = (Transport.EnumLifeType)((int)recordings[recordingIndex].CapturedLifeCycleEvents[vIndex].LifeCycleEvent)
                    });
                }
            }
            return transports;
        ...
        private static Transport.CustomEventCapture[] ToTransport(params CustomEventCapture[] customEvents)
        {
            ...
            for (int eventIndex = 0; eventIndex < customEvents.Length; eventIndex++)
            {
                ...
                transportEvents[eventIndex].Data.Add(customEvents[eventIndex].Contents);
            }
            return transportEvents;
        }
