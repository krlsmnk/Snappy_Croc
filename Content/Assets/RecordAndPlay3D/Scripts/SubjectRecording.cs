...
namespace RecordAndPlay
{
    ...
    public class SubjectRecording : ISerializationCallbackReceiver
    {
        ...
        private float MaxTime<T>(T[] original) where T : Capture
        {
            if (original == null || original.Length == 0)
            {
                return Mathf.NegativeInfinity;
            }
            return original[original.Length-1].Time;
        }
        ...
        /// </summary>
        /// <returns>The time of the last event captured.</returns>
        public float GetEndTime()
        {
            return Mathf.Max(
                MaxTime(capturedPositions),
                MaxTime(capturedRotations),
                MaxTime(capturedLifeCycleEvents),
                MaxTime(capturedCustomSubjectEvents)
            );
        }
