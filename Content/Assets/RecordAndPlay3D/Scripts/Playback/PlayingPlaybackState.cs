...
namespace RecordAndPlay.Playback
{
    ...
    internal class PlayingPlaybackState : PlaybackState
    {
        ...
        {
            if (playbackStateInfo.HasCustomEventHandlers() == false)
            {
                return;
            }
            ...
            foreach (var customEvent in playbackStateInfo.GetRecording().CapturedCustomEvents)
            {
                ...
                {
                    foreach (var handler in playbackStateInfo.GetCustomEventHandlers())
                    {
                        handler.OnCustomEvent(null, customEvent);
                    }
                }
            }
        }
