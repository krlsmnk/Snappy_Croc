...
namespace RecordAndPlay.Record
{
    ...
    public class SubjectBehavior : MonoBehaviour
    {
        ...
        public void CaptureCustomEvent(string name, string contents)
        {
            ...
            subjectRecorder.CaptureCustomEvent(recorder.GetTimeProvider().CurrentTime(), name, contents);
        }
        ...
        void Update()
        {
            ...
            if (framerateToRecordAt < 0 || recorder.GetTimeProvider().CurrentTime() - timeOfLastFrameCapture >= 1f / framerateToRecordAt)
            {
                timeOfLastFrameCapture = recorder.GetTimeProvider().CurrentTime();
        ...
        private void CaptureUnityEvent(UnityLifeCycleEvent e)
        {
            if (subjectRecorder != null)
            {
                subjectRecorder.CaptureUnityEvent(recorder.GetTimeProvider().CurrentTime(), e);
            }
        }
