...
namespace RecordAndPlay.Editor.Extension
{
    ...
    public class PlaybackWindow : EditorWindow
    {
        ...
        public void SetRecordingForPlayback(Recording recording)
        {
            if (playbackService != null)
            {
                playbackService.Stop(ActorCleanupMethod.DestroyImmediate);
                DestroyImmediate(playbackService.gameObject);
            }
        ...
        private int ControlPlaybackOptions(int x, int y)
        {
            ...
            if (playbackService == null)
            {
                ...
                {
                    playbackService = PlaybackBehavior.Build(loadedRecording, useCustomActors ? ActorBuilderReference() : null, null, loop, new Time.RealTimeSinceStartupProvider());
